...
# without categorisation:
 ...
 # BEST COMBO: a=0.05, b=0.5 or a=0.05, b=0.1 (= a=0.1,b=0.1) to be stricter with probs for pos viol
...
obs1[subset1,]
plot(seq_len(nrow(obs1[subset1,])), obs1[subset1,"L"])  
...
# essence: KBSD identified the both critical strata
...
nrow(obs2[obs2$fit == 1 & obs2$A == 1, ])/nrow(obs2[obs2$fit == 1, ])
...
nrow(obs2[obs2$age <= 60 & obs2$fit == 1 & obs2$A == 1,])/nrow(obs2[obs2$age <= 60 & obs2$fit == 1,])  # young & fit rarely treated
  ...
  # only detectable for b = 0.1, sample size sufficient with 521/1000
...
# first categorisation: identical for gamma = 1,2 as only ever split by 1 var
  # too imprecise, only identified any if beta >= 0.05, only viol 2+3 for beta = 0.01
# second categorisation:
  # gamma = 1: same as for first categorisation
  # gamma = 2:
  # for alpha=0.01-0.05, beta=gruber: one split with 2 vars (intersection fit=0 & age=(20,40])
  #   as allows for smaller proba.exposure that only intersection of groups fulfills -> new unexpected subgroup!
  # else split by 1 var, viol #2 detected for beta=0.05, viol 2+3 for beta=0.1
...
table(obs2[obs2$age > 60 & obs2$fit == 0, "A"])/nrow(obs2[obs2$age > 60 & obs2$fit == 0,])
  # indeed critical as proba.exposure = 0.95 -> should've been detected by port for beta = 0.1
