...
# g = 1: only with other cont vars & with v small a=0.01/0.025, but none for recommended a=0.05/0.1
...
port("A", cov.quanti = NULL,
     ...
     data = dat20_cat, alpha = 0.01, beta = 0.1, gamma = 3)
...
dat20 %>% filter(L5 <4 & A==1) %>% nrow()/
  dat20 %>% filter(L5 <4) %>% nrow()  # P(A=1)~1 -> to be det for g>=1, b=0.1, a <= 0.05 (sample prop= 9.9%)
...
sink()
# g = 1: only with other cont vars & only with v small a=0.01/0.025
# g = 2: det from a>=0.025 & b=0.1! alr shows pos eff of cat CONT conf on detection of viol involving CAT conf
...
dat20 %>% filter(L5 > 4 & L5 < 6 & A==1) %>% nrow()/
  dat20 %>% filter(L5 > 4 & L5 < 6) %>% nrow()  # P(A=1)~0 -> to be det for g>=1, any b & any a (sample prop= 16.2%)
