...
obs2 %>% filter(L1 > 60 & A==1) %>% nrow()/obs2 %>% filter(L1 > 60) %>% nrow()  # viol #1: P(A=0)~0 for b=0.1, any a
obs2 %>% filter(L1 < 60 & A==1) %>% nrow()/obs2 %>% filter(L1 < 60) %>% nrow()

obs2 %>% filter(L2 == 0 & A==1) %>% nrow()/obs2 %>% filter(L2 ==0) %>% nrow()  # viol #2: P(A=0)~0 for beta=0.05/0.1, any a
obs2 %>% filter(L1 > 60 & L2 == 0 & A==1) %>% nrow()/obs2 %>% filter(L1 > 60 & L2 ==0) %>% nrow()  # viol #3: combo of the 2 above with beta = 0.01, any a!

obs2 %>% filter(L2 == 1 & A==1) %>% nrow()/obs2 %>% filter(L2 ==1) %>% nrow()  # viol #4: P(A=1)~0 for beta=0.1, any a
...
sem3 <- DAG.empty() +
  ...
  node("A", distr = "rbern", prob = 0.26*L1 + 0.31*L2 + 0.42*L3)  # if L1, L2, L3=1 -> viol for A=0, if L3=0 -> viol for A=1
dag3 <- set.DAG(sem3)
plotDAG(dag3)
obs3 <- sim(dag3, rndseed = 12082025, n = 1000)
table(obs3$A)  
...
#      L2=0 & L3=0 -> viol #3: P(A=1)~0 for b=0.1 (34.6%) -> covered by viol #1
# g=3: L1=1 & L2=1 & L3=1 -> viol #4: P(A=1)~1 for a=0.01 & b=0.1 
#      L1=0 & L2=0 & L3=0 -> viol #5: P(A=1)~0 for any a & b -> covered by viol #1
...
res3_plot <- kbsd(data = o3,
                  ...
                  disthalf_vec=c(L1=sd(o3$L1), L2 = sd(o3$L2), L3 = sd(o3$L3), A=0.5*sd(o3$A)))  # use 1 SD for L_i, 0.5 SD for A
res3_plot 
#res3_plot / plot_spacer()
#ggsave("bp3_binary.png", width = 2.5, height = 8)
...
# identify obs with low EDP
indices_outliers <- unique(res3[res3$shift == 1 & res3$diagnostic < quantile(res3$diagnostic, 0.05), "observation"])
o3[indices_outliers, ] %>% nrow()
o3[indices_outliers, ] %>% filter(L3==0) %>% nrow()
indices_outliers_0 <- unique(res3[res3$shift == 2 & res3$diagnostic < quantile(res3$diagnostic, 0.05), "observation"])
o3[indices_outliers_0, ] %>% nrow()
o3[indices_outliers_0, ] %>% filter(!(L1==1 & L2==1 & L3==1)) %>% nrow()
# q=0.05: (18+40)/90=64% of obs with few EDP are from critical stratum -> majority
# q=0.01: all with low EDP were from the 2 viol strata
o3[indices_outliers, ] %>% filter(!((L1==1 & L2==1 & L3 ==1) | L3==0))
# q=0.05. most of rest is L1==0 & L2==1 & L3==1 -> check if really a new viol stratum!
o3[indices_outliers, ] %>% filter(L1==0 & L2==1 & L3==1 & A==1) %>% nrow()/
  o3[indices_outliers, ] %>% filter(L1==0 & L2==1 & L3==1) %>% nrow()  # no, not a viol
